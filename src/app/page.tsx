...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900 selection:bg-blue-600/10">
      ...
      {/* Header */}
      <header className="h-20 border-b border-zinc-200 flex items-center justify-between px-12 relative z-50 bg-white/80 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center shadow-lg shadow-blue-600/20">
            <div className="w-4 h-4 bg-white rounded-sm"></div>
          </div>
          <span className="font-black uppercase tracking-widest text-sm text-zinc-900">AssetPacker<span className="text-blue-500">.ai</span></span>
        </div>

        <nav className="flex items-center gap-2 p-1.5 bg-zinc-100 rounded-2xl border border-zinc-200">
           <button 
            onClick={() => setActiveTab('packer')}
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase transition-all duration-300 ${activeTab === 'packer' ? 'bg-white text-blue-600 shadow-md ring-1 ring-zinc-200' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50'}`}
           >
           ...
           <button 
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase transition-all duration-300 ${activeTab === 'settings' ? 'bg-white text-blue-600 shadow-md ring-1 ring-zinc-200' : 'text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50'}`}
           >
