...
export const stockService = {
  async search(query: string, pexelsKey: string, pixabayKey: string): Promise<StockAsset[]> {
    const pexelsUrl = `https://api.pexels.com/v1/search?query=${encodeURIComponent(query)}&per_page=10`;
    const pexelsVideoUrl = `https://api.pexels.com/videos/search?query=${encodeURIComponent(query)}&per_page=10`;
    const pixabayUrl = `https://pixabay.com/api/?key=${pixabayKey}&q=${encodeURIComponent(query)}&image_type=all&per_page=10`;
